...
parfor j=1:N
    ...
    for i=1:NT
        ...
        dx=sqrt(2*D*dt)*x^(exp1)*randn+2*D*(exp1)*x^(exp2)*dt;
        x=x+dx;
        %---if you use
        %---x=x+sqrt(D*dt)*randn;
...
etime=toc;
%save("Documents/MATLAB/WS_Diff_coeff_HK.mat");